    ...
    <el-dialog :visible.sync="dialogVisible" :title="dialogType==='edit'?'Edit Menu':'New Menu'">
      <el-form ref="formData" :model="Menu" :rules="rules" label-width="80px" label-position="left">
        ...
        <el-form-item label="状态">
          <el-switch v-model="Menu.status" :on-value="true" :off-value="false" />
        </el-form-item>
...
import { getMenus, addMenu, updateMenu, deleteMenu } from '@/api/user'
const defaultMenu = {
  id: '',
  name: '',
  path: '',
...
export default {
  ...
  methods: {
    ...
    handleEdit(scope) {
      ...
      this.Menu.status = (this.Menu.status === 1)
      this.Menu.meta_nocache = (this.Menu.meta_nocache === 1)
      this.Menu.hidden = (this.Menu.hidden === 1)
      this.Menu.alwaysshow = (this.Menu.alwaysshow === 1)
      this.$nextTick(() => {
        this.$refs['formData'].clearValidate()
      })
    },
